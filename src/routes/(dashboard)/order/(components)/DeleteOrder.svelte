<script lang="ts">
    ...
    const handleDeleteTransaction:  SubmitFunction = ( { formData, cancel } ) => {
        if(!transactionId) return cancel()
...
<Dialog.Root open={isOpen} onOpenChange={closeModal}>
    <Dialog.Content>
        ...
        <form action="?/deleteOrder" method="post" use:enhance={handleDeleteTransaction}>
            <Dialog.Footer class="flex gap-2 items-center justify-between">
                <Button variant="outline" type="button" on:click={closeModal}>Batal</Button>
