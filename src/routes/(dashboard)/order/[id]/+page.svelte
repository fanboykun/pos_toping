<script lang="ts">
	...
	import { makeTransaction, prepareMakeTransaction, updateOrAddProduct, updateOrAddTopping, type MakeTransactionProduct, type MakeTransactionToping } from '$lib/transaction.js';
	...
	import ProductCategoryList from '../add/(components)/ProductCategoryList.svelte';
	import ProductList from '../add/(components)/ProductList.svelte';
	import OrderMenu from '../add/(components)/OrderMenu.svelte';
    ...
    onMount(async () => {
        if(data.transaction){
            $makeTransaction = prepareMakeTransaction(data.transaction)
        } else {
    ...
    const handleEditTransaction: SubmitFunction = ( { formData, cancel } ) => {
        processing = true
        if(!data.transaction) return cancel()
...
<div class="grid grid-cols-4 md:grid-cols-12 gap-2 w-full p-4 h-auto min-h-[80svh] max-h-[90svh] place-content-start">
    ...
    <div in:horizontalSlide={{ ...transitionProps }} class="col-span-4 md:col-span-5 xl:col-span-4 w-full h-full gap-4 justify-between p-2 border shadow-lg rounded-xl {selectedMenu == 'cart' ? 'flex flex-col' : 'hidden md:flex flex-col'}">
        ...
        <div class="w-full h-fit items-end justify-end">
            <form action="?/editTransaction" use:enhance={handleEditTransaction} method="post">
