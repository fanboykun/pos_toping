<script lang="ts">
    ...
    const handleDeleteCategory:  SubmitFunction = ( { formData, cancel } ) => {
        if(!categoryId) return cancel()
...
<Dialog.Root open={isOpen} onOpenChange={closeModal}>
    <Dialog.Content>
        ...
        <form action="?/deleteCategory" method="post" use:enhance={handleDeleteCategory}>
