          ...
          <!-- Table -->
           <div class="grid grid-cols-1 w-full sm:grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4 p-2">
             ...
             <ProductCard>
               ...
               <div slot="action">
                 {#if data.user?.isAdmin}
                 <button type="button" on:click={() => { isDeleteProductModalOpen = true; selectedProduct = product }} class="p-2 mt-3 inline-flex items-center gap-x-0 text-sm font-semibold rounded-lg border border-red-400 text-red-600 decoration-2 hover:text-red-100 hover:bg-red-600 focus:shadow-md disabled:opacity-50 disabled:pointer-events-none">
                ...
                <button type="button" on:click={() => { isEditProductModalOpen = true; selectedProduct = product }} class="p-2 mt-3 inline-flex items-center gap-x-0 text-sm font-semibold rounded-lg border border-blue-400 text-blue-600 decoration-2 hover:text-blue-100 hover:bg-blue-600 focus:shadow-md disabled:opacity-50 disabled:pointer-events-none">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="shrink-0 size-4">
...
<DeleteProduct isOpen={isDeleteProductModalOpen} productId={selectedProduct?.id} {form} onClose={() => onActionModalCLose('delete')} />
  {#await data.categories }
    <div class="sr-only">loading modal</div>
  {:then categories}
  {#if categories}
