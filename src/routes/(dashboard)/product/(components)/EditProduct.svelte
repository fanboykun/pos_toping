<script lang="ts">
  ...
  let selectedcatId: unknown = ''

  $: selectedCat = {
		label: capitalizeFirstLetterOfEachWord(product?.category?.name ?? ''),
  ...
  const handleEditProduct: SubmitFunction = ( { formData, cancel } ) => {
      if(!product) return cancel()
      editing = true
...
<Dialog.Root open={isOpen} onOpenChange={closeModal}>
    <Dialog.Content class="sm:max-w-[425px]">
      ...
      <form method="post" action="?/editProduct" use:enhance={handleEditProduct} id="addProduct">
        <div class="grid w-full items-center gap-4">
          ...
          <div class="flex flex-col space-y-1.5">
            ...
            <Input id="name" name="name" placeholder="Nama Produk" value={product?.name ?? ''} />
          ...
          <div class="flex flex-col space-y-1.5">
            ...
            <Input id="price" name="price" placeholder="Harga Produk" type="number" value={product?.price} />
          ...
          <div class="flex flex-col space-y-1.5">
            ...
            <Select.Root name="categoryId" selected={selectedCat}>
        ...
        <Dialog.Footer class="flex justify-between mt-4">
          ...
          <Button type="submit" disabled={editing} class="flex gap-1">
            <span>Update</span>
            {#if editing}
