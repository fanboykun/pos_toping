<script lang="ts">
    ...
    const findIsActiveRoute = (menu: Menu): boolean => {
        if(menu.isRoot){ return $page.url.pathname == menu.path }
        else { return $page.url.pathname.includes(menu.path) }
    }
...
<!-- ========== HEADER ========== -->
<header class="sticky top-0 inset-x-0 flex flex-wrap justify-center  md:flex-nowrap z-50 w-full text-sm">
    <nav class="mt-4 relative max-w-2xl w-full bg-white border border-gray-200 rounded-[2rem] mx-2 py-2.5 md:flex md:items-center md:justify-between md:py-0 md:px-4 md:mx-auto dark:bg-neutral-900 dark:border-neutral-700">
        ...
        {#key openMobileMenu}
        <div transition:horizontalSlide={{ delay: 100, duration: 500, axis: 'y' }} class=" {openMobileMenu ? 'block' : 'hidden md:block'} hs-collapse overflow-hidden transition-all duration-300 basis-full grow">
            
            <!-- Links -->
            <div class="flex flex-col md:flex-row md:items-center md:justify-end gap-2 md:gap-3 mt-3 md:mt-0 py-2 md:py-0 md:ps-7">
                {#key $page.url.pathname}
                    {#each menus as menu}
                        <a class="py-0.5 md:py-3 px-4 md:px-1 border-s-2 md:border-s-0 md:border-b-2 focus:outline-none {findIsActiveRoute(menu) ? 'active' : 'inactive'}" href={menu.path} aria-current="page">{menu.label}</a>
                    {/each}
                {/key}

                {#if user}
                    <Avatar {user} shouldHideOnMobile={true} />
                {/if}
            </div>
            <!-- End Links -->
...
<style>
    .active {
        @apply border-gray-800 font-medium text-gray-800;
    }
    .inactive {
        @apply border-transparent text-gray-500 hover:text-gray-800;
    }
</style>